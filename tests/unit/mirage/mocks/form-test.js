...
module("Unit | Mirage GraphQL Mock | form", function(hooks) {
  ...
  test("can reorder questions", async function(assert) {
    ...
    const res = await this.apollo.mutate({
      ...
      variables: {
        input: {
          form: btoa("Form:test-form"),
          questions: [
            btoa("TextQuestion:question-2"),
            btoa("TextQuestion:question-3"),
            btoa("TextQuestion:question-1")
          ]
        }
      }
    });
  ...
  test("can add question", async function(assert) {
    ...
    const res = await this.apollo.mutate({
      ...
      variables: {
        input: {
          form: btoa("Form:test-form"),
          question: btoa("TextQuestion:test-question")
        }
      }
    });
  ...
  test("can remove question", async function(assert) {
    ...
    const res = await this.apollo.mutate({
      ...
      variables: {
        input: {
          form: btoa("Form:test-form"),
          question: btoa("TextQuestion:question-1")
        }
      }
    });
